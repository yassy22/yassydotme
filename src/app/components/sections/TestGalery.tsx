...
// TIP: hernoem het bestand én de export naar TestGallery om verwarring te voorkomen
export default function TestGallery() {
  ...
  const rise1: Variants = {
    ...
    animate: {
      ...
      transition: {
        duration: 1,
        ease: cubicBezier(0.83, 0, 0.17, 1), // <-- typesafe
        delay: 0.3,
      },
    },
  };
  ...
  const animate = () => {
    ...
    if (xForce !== 0 || yForce !== 0) {
      requestAnimationFrameId = requestAnimationFrame(animate);
    } else if (requestAnimationFrameId !== null) {
      cancelAnimationFrame(requestAnimationFrameId);
      requestAnimationFrameId = null;
    }
  };

  // about[..].collomnX kan string of StaticImageData zijn
  const col1 = (about?.[0]?.collomn1 ?? []) as (string | StaticImageData)[];
  const col2 = (about?.[0]?.collomn2 ?? []) as (string | StaticImageData)[];
      ...
      <div className="hero-about h-screen flex justify-center items-center ">
        <div className="hero-about__content text-center overflow-hidden">
          <motion.h1
            ...
            initial="initial"
            animate="animate"
            className={`${NewYork.className} dark:mix-blend-exclusion text-[12vw] xs:text-[9vw] sm:text-[8vw] lg:text-[5vw] text-center leading-[1] text-[#FCFAF6]`}
          >
